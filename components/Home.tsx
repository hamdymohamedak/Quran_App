...
const Home = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        let api = 'http://api.alquran.cloud/v1/quran/ar.alafasy';
  ...
  return (
    <View style={styles.parent}>
      ...
      <View style={styles.imageContainer}>
        ...
        <Text style={styles.overlayText}>Quran App</Text>
        <Text style={styles.overlayDate}>{todayDate}</Text>
      </View>
      <View style={styles.QuranBar}>
        <TouchableOpacity>
          <Text
            onPress={() => navigation.navigate('TimingScreen')}
        ...
        <TouchableOpacity
          onPress={() => {
            navigation.navigate('salahWay');
          }}>
          <Text style={{fontWeight: 'bold'}}>طريقة الصلاة</Text>
        </TouchableOpacity>
        <TouchableOpacity
          onPress={() => {
            navigation.navigate('Wash');
          }}>
          <Text style={{fontWeight: 'bold'}}>الوضوء</Text>
        </TouchableOpacity>
        <TouchableOpacity onPress={()=>{
          navigation.navigate('EslamCummunity')
        }}>
          <Text style={{fontWeight: 'bold'}}>المجتمعات الاسلاميه</Text>
        </TouchableOpacity>
      </View>
        ...
        {apiData.length === 0 ? (
          ...
          apiData.map((info, index) => (
            <TouchableOpacity
              onPress={() => {
                let ayahText = '';
                info.ayahs.forEach(ayah => {
                  ayahText += ayah.text + '\n';
                });
                let SuraName = info.name;
              ...
              <View style={styles.boxParent}>
                <Text style={styles.box}>{index + 1}</Text>
              </View>
            </TouchableOpacity>
          ))
        )}
